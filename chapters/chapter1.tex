...
The reason behind the focus on maximizing memory efficiency is related to the fact that memory represents the most expensive constraint in our ideal target deployment scenario, where the hardware typically consists of a low-power RISC-V SoC with severely constrained, albeit exceptionally fast, memory resources (further details on the representative target hardware can be found in Section \ref{target_hardware}). Such specialized architectures are designed to address the fundamental challenges of local AI deployment, prioritizing privacy preservation and energy efficiency over raw computational power. These systems achieve efficient on-device inference through purpose-built hardware components, including dedicated neural processing units and optimized memory hierarchies that maximize bandwidth while minimizing power consumption. The underlying philosophy mirrors this thesis in its commitment to enabling practical local AI execution, though through a fundamentally different approach: while the hardware focuses on architectural innovations and specialized silicon design, this work tackles the same deployment challenges through algorithmic model optimization. Crucially, optimizations that reduce memory footprint also translate directly into computational complexity improvements, creating a dual benefit for resource-constrained environments.
